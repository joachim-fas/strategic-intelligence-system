...
export default function ScenarioPanel({ trends, onApplyScenario, onReset }: ScenarioPanelProps) {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        style={{ background: "#FFF5BA", color: "#7A5C00", border: "1px solid #E0C840" }}
      >
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="sticky top-0 z-10 px-6 py-4 border-b" style={{ background: "#FFFFFF", borderColor: "#E8E8E8" }}>
        <div className="flex items-center justify-between">
          <h2 className="text-base font-semibold text-[#1A1A1A]">
            {locale === "de" ? "⚡ What-If Szenarien" : "⚡ What-If Scenarios"}
          </h2>
          <button onClick={() => setIsOpen(false)} className="text-[#9B9B9B] hover:text-[#1A1A1A] text-sm transition-colors">
            ✕
          </button>
        </div>
        <p className="text-xs text-[#9B9B9B] mt-1">
          {locale === "de"
            ? "Wie verändern sich die Trends wenn ein Ereignis eintritt?"
        ...
        {activeResult && (
          <button
            ...
            style={{ background: "#FDEEE9", color: "#C0341D", border: "1px solid #F4A090" }}
          >
            {locale === "de" ? "↺ Szenario zurücksetzen" : "↺ Reset scenario"}
          </button>
        )}
      </div>
      ...
      {/* Active Result */}
      {activeResult && (
        <div className="px-6 py-4 border-b" style={{ background: "#FDE2FF", borderColor: "#D4A0F0" }}>
          <h3 className="text-sm font-semibold mb-3" style={{ color: "#7C1A9E" }}>
            {activeResult.scenario.name}
          </h3>
          <div className="grid grid-cols-3 gap-3 mb-3">
            <div className="text-center">
              <div className="text-lg font-bold text-[#1A1A1A]">{activeResult.totalAffected}</div>
              <div className="text-[10px] text-[#9B9B9B]">
                {locale === "de" ? "Betroffen" : "Affected"}
              </div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold" style={{ color: "#7A5C00" }}>{activeResult.ringChanges}</div>
              <div className="text-[10px] text-[#9B9B9B]">
                {locale === "de" ? "Ring-Wechsel" : "Ring changes"}
              </div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold" style={{ color: "#7C1A9E" }}>
              ...
              return (
                <div key={at.trendId} className="flex items-center gap-2 text-xs">
                  <span
                    ...
                    style={{ background: at.impactType === "direct" ? "#7C1A9E" : "#D4A0F0" }}
                  />
                  <span className="text-[#3A3A3A] flex-1 truncate">{at.trendName}</span>
                  {at.originalRing !== at.newRing && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium"
                      style={{ background: "#FFF5BA", color: "#7A5C00" }}>
                      {at.originalRing} → {at.newRing}
                    </span>
                  )}
                  <span className="text-[10px] font-mono" style={{
                    color: shift > 0 ? "#0F6038" : shift < 0 ? "#C0341D" : "#9B9B9B"
                  }}>
      ...
      <div className="px-6 py-5">
        <h3 className="text-[10px] font-semibold text-[#9B9B9B] uppercase tracking-wider mb-3">
          {locale === "de" ? "Szenarien auswählen" : "Select Scenario"}
        </h3>
        <div className="space-y-2.5">
          {SCENARIO_TEMPLATES.map((template, i) => {
            ...
            const probColor = template.probability > 0.5
              ? { bg: "#C3F4D3", color: "#0F6038" }
              : template.probability > 0.2
              ? { bg: "#FFF5BA", color: "#7A5C00" }
              : { bg: "#FDEEE9", color: "#C0341D" };
            return (
              <button
                key={i}
                ...
                style={isActive
                  ? { background: "#FDE2FF", borderColor: "#D4A0F0" }
              ...
              >
                ...
                <p className="text-[11px] text-[#6B6B6B] line-clamp-2 leading-relaxed">{template.description}</p>
                <div className="mt-2 flex items-center gap-2">
                  <span className="text-[10px] text-[#9B9B9B]">
                    {template.directImpacts.length} {locale === "de" ? "direkte Auswirkungen" : "direct impacts"}
                  </span>
                  {template.source && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#F0F2F7] text-[#3A4560]">
                      {template.source}
                    </span>
                  )}
                </div>
