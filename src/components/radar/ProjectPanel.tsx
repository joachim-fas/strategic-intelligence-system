...
export default function ProjectPanel({ locale, activeProjectId, onSelectProject, onClose }: ProjectPanelProps) {
  ...
  return (
    <div className="fixed right-0 top-0 bottom-0 w-[400px] z-40 flex flex-col shadow-xl"
      style={{ background: "#FAFAFA", borderLeft: "1px solid #E8E8E8" }}>
        ...
        {view === "detail" && selectedProjectData ? (
          <>
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-sm font-semibold text-[#1A1A1A]">{selectedProjectData.name}</span>
              <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium"
                style={{ background: "#C3F4D3", color: "#0F6038" }}>
                {de ? "Aktiv" : "Active"}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <button
                ...
                className="text-[11px] px-2.5 py-1 rounded-full border text-[#6B6B6B] hover:text-[#1A1A1A] hover:border-[#1A1A1A] transition-colors"
                style={{ borderColor: "#E0E0E0", background: "white" }}
              >
                Export
              </button>
              <button onClick={onClose} className="text-[#9B9B9B] hover:text-[#1A1A1A] text-sm transition-colors">✕</button>
            </div>
          </>
        ) : (
          <>
            <div className="flex items-center gap-2">
              <span className="text-sm font-semibold text-[#1A1A1A]">{de ? "Projekte" : "Projects"}</span>
              {projects.length > 0 && (
                <span className="text-[10px] px-1.5 py-0.5 rounded-full font-mono font-medium"
                  style={{ background: "#F0F2F7", color: "#6B7A9A" }}>
                  {projects.length}
                </span>
              )}
            </div>
          ...
          <div className="px-5 py-4 border-b" style={{ borderColor: "#F0F0F0" }}>
            <div className="flex gap-2">
              <input
                ...
                onBlur={(e) => { e.currentTarget.style.borderColor = "#E8E8E8"; e.currentTarget.style.background = "#F5F5F5"; }}
              />
              <button
                onClick={createProject}
                className="px-3 py-2 rounded-lg text-xs font-semibold transition-colors"
                style={{ background: "#E4FF97", color: "#0A0A0A", border: "1px solid rgba(0,0,0,0.08)" }}
              >
                +
              </button>
            </div>
            ...
            {projects.map((p) => (
              <button
                ...
                style={activeProjectId === p.id
                  ? { background: "#C3F4D3", borderColor: "#6FD99A" }
                  : { background: "#FFFFFF", borderColor: "#E8E8E8" }}
              >
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-sm font-medium text-[#1A1A1A]">{p.name}</span>
                  <span className="text-[10px] text-[#9B9B9B]">{timeAgo(p.updated_at)}</span>
                </div>
                {p.description && (
                  <p className="text-[11px] text-[#6B6B6B] mb-1.5 line-clamp-1">{p.description}</p>
                )}
      ...
      {view === "detail" && selectedProject && (
        <div className="flex-1 overflow-y-auto">
          {/* Stats bar */}
          <div className="px-5 py-2.5 border-b flex items-center gap-3 text-[11px] text-[#9B9B9B]"
          ...
          <div className="px-5 py-3 border-b" style={{ borderColor: "#F0F0F0" }}>
            <div className="flex gap-2">
              <input
                ...
                style={{ background: "#F5F5F5", border: "1px solid #E8E8E8", color: "#1A1A1A" }}
              />
              <button
                onClick={() => addNote()}
            ...
            {notes.filter((n) => !n.query_id).map((n) => (
              <div key={n.id} className="flex items-start gap-2 mt-2 group">
                <span className="text-[10px] text-[#9B9B9B] mt-0.5">📝</span>
                <span className="text-[11px] text-[#3A3A3A] flex-1 leading-relaxed">{n.content}</span>
                <button
                  onClick={() => deleteNote(n.id)}
                  className="text-[10px] text-[#C0C0C0] hover:text-[#E8402A] opacity-0 group-hover:opacity-100 transition-all"
                >✕</button>
              </div>
            ))}
          </div>
            ...
            {queries.map((q) => {
              ...
              return (
                <div key={q.id} className="rounded-xl border overflow-hidden transition-all"
                  style={{ borderColor: isExpanded ? "#D0D0D0" : "#E8E8E8", background: "#FFFFFF" }}>
                  {/* Query header */}
                  <button
                    onClick={() => setExpandedQuery(isExpanded ? null : q.id)}
                    className="w-full text-left px-3.5 py-3 transition-colors hover:bg-[#FAFAFA]"
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-[12px] font-medium text-[#1A1A1A] truncate flex-1">{q.query}</span>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-[10px] text-[#9B9B9B]">{timeAgo(q.created_at)}</span>
                        <span className="text-[10px] text-[#C0C0C0]">{isExpanded ? "▾" : "▸"}</span>
                      </div>
                    </div>
                    {q.result?.synthesis && (
                      <p className="text-[10px] text-[#9B9B9B] mt-1 line-clamp-2 leading-relaxed text-left">
                        {q.result.synthesis}
                      </p>
                    )}
                  </button>
                  ...
                  {/* Expanded content */}
                  {isExpanded && q.result && (
                    <div className="px-3.5 pb-3.5 border-t" style={{ borderColor: "#F0F0F0" }}>
                      {/* Full synthesis */}
                      <p className="text-[11px] text-[#3A3A3A] mt-2.5 leading-relaxed">{q.result.synthesis}</p>
                      ...
                      {q.result.scenarios?.length > 0 && (
                        <div className="mt-3 space-y-1.5">
                          ...
                          {q.result.scenarios.map((s: any, i: number) => {
                            ...
                            const color = pct > 40
                              ? { bg: "#C3F4D3", text: "#0F6038" }
                              : pct > 25
                              ? { bg: "#FFF5BA", text: "#7A5C00" }
                              : { bg: "#FDEEE9", text: "#C0341D" };
                            return (
                              <div key={i} className="flex items-center gap-2 text-[10px]">
                                ...
                                <span className="text-[#3A3A3A] font-medium">{s.name}:</span>
                                <span className="text-[#6B6B6B] truncate">{s.description?.slice(0, 60)}…</span>
                              </div>
                            );
                          })}
                        </div>
                        ...
                        <div className="mt-3 space-y-1">
                          {q.result.keyInsights.slice(0, 3).map((insight: string, i: number) => (
                            <div key={i} className="flex items-start gap-2 text-[10px]">
                              <span className="text-[#E4FF97] bg-[#0A0A0A] rounded px-1 font-bold flex-shrink-0 mt-0.5">→</span>
                              <span className="text-[#6B6B6B] leading-relaxed">{insight}</span>
                            </div>
                          ))}
                        </div>
                      )}
                      ...
                      {notes.filter((n) => n.query_id === q.id).map((n) => (
                        <div key={n.id} className="flex items-start gap-1.5 mt-2 group">
                          ...
                          <span className="text-[10px] text-[#3A3A3A] flex-1">{n.content}</span>
                          <button
                            onClick={() => deleteNote(n.id)}
                            className="text-[10px] text-[#C0C0C0] hover:text-[#E8402A] opacity-0 group-hover:opacity-100 transition-all"
                          >✕</button>
                      ...
                      {/* Actions */}
                      <div className="flex items-center gap-3 mt-3 pt-2 border-t" style={{ borderColor: "#F0F0F0" }}>
                        <button
                          onClick={() => deleteQuery(q.id)}
                          className="text-[10px] text-[#9B9B9B] hover:text-[#E8402A] transition-colors"
                        >
                          {de ? "Löschen" : "Delete"}
                        </button>
                      </div>
